...
def _ensure_schema():
    ...
    # Ensure question_set table exists and question.question_set_id column exists
    try:
        db.session.execute('SELECT id FROM question_set LIMIT 1')
    except Exception as e:
        # If the table doesn't exist, create all missing tables (will create question_set)
        try:
            db.create_all()
            print('Created question_set table via create_all fallback')
        except Exception:
            pass
        try:
            db.session.rollback()
    ...
    try:
        db.session.execute('SELECT question_set_id FROM question LIMIT 1')
    except Exception as e:
        msg = str(e).lower()
        if 'no such column' in msg and 'question_set_id' in msg:
            try:
                ...
                print('Added question.question_set_id column via fallback ALTER')
            except Exception as _:
                print('Failed to add question.question_set_id column:', str(_))
        try:
            db.session.rollback()
        except Exception:
            pass
...
class Question(db.Model):
    ...
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    # Optional grouping to keep uploaded/question sets standalone
...
# Ensure minimal schema for question sets exists at import time so request
# handlers that reference `question.question_set_id` don't fail with
# sqlite "no such column" when tests or the app import this module.
try:
    # Create any missing tables (will not alter existing ones)
    try:
        db.create_all()
    except Exception:
        pass
    # Check question table columns and add question_set_id if missing
    try:
        conn = db.engine.connect()
        try:
            res = conn.execute("PRAGMA table_info('question')")
            cols = [r[1] for r in res.fetchall()]
            if 'question_set_id' not in cols:
                try:
                    conn.execute('ALTER TABLE question ADD COLUMN question_set_id INTEGER')
                    print('Added `question_set_id` column to question table at import-time')
                except Exception:
                    pass
        finally:
            conn.close()
    except Exception:
        # If any DB engine isn't ready at import-time, skip silently.
        pass
except Exception:
    pass
...
class Recording(db.Model):
    ...
    # Student class label for the recording (e.g. SS1, JSS2)
    student_class = db.Column(db.String(50), nullable=True)
    uploaded_at = db.Column(db.DateTime, default=datetime.utcnow)


class QuestionSet(db.Model):
    """Represents a named set/batch of uploaded questions so admins can create
    exams from a specific upload without mixing with the global pool."""
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(200), nullable=False)
    subject_id = db.Column(db.Integer, db.ForeignKey('subject.id'))
    subject = db.relationship('Subject', backref='question_sets')
    created_by = db.Column(db.Integer, db.ForeignKey('user.id'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    upload_filename = db.Column(db.String(300))


class CommunityPost(db.Model):
    """Simple global community posts visible to everyone. No auth required to view/post."""
    id = db.Column(db.Integer, primary_key=True)
    ...
    # Optional attachment filename (stored under uploads/community/)
    attachment = db.Column(db.String(400), nullable=True)
...
def init_db():
    with app.app_context():
        ...
        try:
            ...
            pass
        # Ensure recording table has student_class column
        try:
            from sqlalchemy import inspect as _inspect
            inspector2 = _inspect(db.engine)
            if 'recording' in inspector2.get_table_names():
                rcols = [c['name'] for c in inspector2.get_columns('recording')]
                if 'student_class' not in rcols:
                    try:
                        _exec_ddl("ALTER TABLE recording ADD COLUMN student_class VARCHAR(50)")
                        print('Added student_class to recording table')
                    except Exception:
                        pass
        except Exception:
            pass
        # Ensure exam table has allowed_classes column
        try:
            from sqlalchemy import inspect as _inspect3
            inspector3 = _inspect3(db.engine)
            if 'exam' in inspector3.get_table_names():
                ecols = [c['name'] for c in inspector3.get_columns('exam')]
                if 'allowed_classes' not in ecols:
                    try:
...
def admin_dashboard():
    ...
    subjects = subjects_for_current_user()
    # Prepare `classes` early so it's available to any error/return branches
    try:
    ...
    for c in existing_classes + common_classes:
        if c and c not in classes:
            classes.append(c)
    # Ensure `classes` is available early so error branches can reference it
    try:
        existing_classes = [s.subject_class for s in subjects if getattr(s, 'subject_class', None)]
    except Exception:
        existing_classes = []
    ...
    # Prepare a default `classes` list early so error paths can safely reference it
    try:
        existing_classes = [s.subject_class for s in subjects if getattr(s, 'subject_class', None)]
    except Exception:
        existing_classes = []
    ...
    # determine admin's school id for class scoping
...
@app.route('/admin/recordings/delete', methods=['POST'])
def admin_delete_recordings():
    if 'user_id' not in session or session.get('role') != 'admin':
        flash('Access denied', 'danger')
        return redirect(url_for('login'))

    ids = request.form.getlist('selected_recordings')
    if not ids:
        flash('No recordings selected for deletion', 'warning')
        return redirect(url_for('admin_dashboard'))

    deleted = 0
    for rid in ids:
        try:
            rec = Recording.query.get(int(rid))
        except Exception:
            rec = None
        if not rec:
            continue
        # Only allow deletion for recordings belonging to exams in admin's scope
        try:
            # ensure admin has access to the exam (unless superadmin)
            if not session.get('is_superadmin'):
                admin_school = _get_effective_school_id()
                sess = ExamSession.query.get(rec.exam_session_id) if rec.exam_session_id else None
                exam = Exam.query.get(sess.exam_id) if sess else None
                if exam and exam.created_by:
                    creator = User.query.get(exam.created_by)
                    if creator and getattr(creator, 'school_id', None) != admin_school:
                        # skip deletion of recordings outside admin's school
                        continue
        except Exception:
            pass
        ...
        try:
            import ntpath
            fname = ntpath.basename(rec.filename or '')
            fpath = os.path.join(app.config.get('UPLOAD_FOLDER', 'uploads'), 'recordings', fname)
    ...
    flash(f'Deleted {deleted} recording(s).', 'success' if deleted else 'info')
    return redirect(url_for('admin_dashboard'))


@app.route('/admin/recording/<int:rec_id>/download')
def admin_download_recording(rec_id):
    if 'user_id' not in session or session.get('role') != 'admin':
        flash('Access denied', 'danger')
        return redirect(url_for('login'))
    ...
    # ensure admin has access to this recording (unless superadmin)
    try:
        if not session.get('is_superadmin'):
            admin_school = _get_effective_school_id()
            sess = ExamSession.query.get(rec.exam_session_id) if rec.exam_session_id else None
    ...
    try:
        ...
        if not os.path.exists(fpath):
            flash('Recording file not found on disk', 'warning')
            return redirect(url_for('admin_dashboard'))
        mtype, _ = mimetypes.guess_type(fpath)
        if not mtype:
            mtype = 'application/octet-stream'
        return send_file(fpath, mimetype=mtype, as_attachment=True, download_name=fname)
    except Exception:
        flash('Unable to download recording', 'danger')
        return redirect(url_for('admin_dashboard'))
...
def admin_questions():
    ...
    if subject_code or subject_class:
        ...
        # No filters — show questions for current user's school
        if session.get('is_superadmin'):
            ...
            if not school_id:
                ...
                # Get all subjects belonging to this school (regardless of who created them)
                school_subjects = Subject.query.join(User, Subject.created_by == User.id).filter(User.school_id == school_id).all()
                subject_ids = [s.id for s in school_subjects]
                if not subject_ids:
                    questions = []
                else:
    ...
    # Group questions by (date, subject, class, question_set name)
    groups_map = {}
    try:
        ...
        for q in questions:
            try:
                date_str = q.created_at.strftime('%Y-%m-%d') if getattr(q, 'created_at', None) else 'Unknown Date'
            except Exception:
                date_str = 'Unknown Date'
            subj_name = q.subject.name if getattr(q, 'subject', None) else ''
            cls = (q.subject_class or (q.subject.subject_class if getattr(q, 'subject', None) else '') ) or ''
            qset_id = getattr(q, 'question_set_id', None)
            qset_name = ''
            try:
                if qset_id:
                    qs = QuestionSet.query.get(qset_id)
                    qset_name = qs.name if qs else ''
            except Exception:
                qset_name = ''
            key = (date_str, subj_name, cls, qset_name)
            groups_map[key].append(q)
        groups = []
        for (date_str, subj_name, cls, qset_name), qlist in groups_map.items():
            groups.append({'date': date_str, 'subject': subj_name, 'class': cls, 'set_name': qset_name, 'questions': qlist})
        # sort groups by date desc then subject
        try:
    ...
    total_questions = sum(len(g['questions']) for g in groups) if groups else len(questions)

    return render_template('admin/questions.html', groups=groups, subjects=subjects, selected_subject=selected_subject, total_questions=total_questions)
        ...
        if 'question_image' in request.files:
            ...
            if qf and qf.filename:
                try:
                    ...
                    qf.save(qpath)
                    question.question_image = os.path.relpath(qpath, app.config['UPLOAD_FOLDER']).replace('\\', '/')
                except Exception:
                    pass
...
def upload_questions():
    ...
    subjects = subjects_for_current_user()
    # Ensure `classes` is defined immediately to avoid UnboundLocalError
    try:
        existing_classes = [s.subject_class for s in subjects if getattr(s, 'subject_class', None)]
    except Exception:
        existing_classes = []
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                for img in images:
                    if img and img.filename:
                        ...
                        img.save(img_path)
                        uploaded_images[img_name] = os.path.relpath(img_path, app.config['UPLOAD_FOLDER']).replace('\\', '/')
            except Exception:
                uploaded_images = {}
            ...
            try:
                ...
                # 1) MCQ format with columns: Question, Option A, Option B, Correct Answer
                # 2) Theory-only format with columns (case-insensitive): S/N, THEORY
                cols_norm = [str(c).strip().upper() for c in df.columns]
                is_theory_only_format = False
                if set(cols_norm) >= {'S/N', 'THEORY'} and 'QUESTION' not in cols_norm:
                    is_theory_only_format = True

                # If not theory-only, require MCQ columns
                if not is_theory_only_format:
                    required_cols = ['Question', 'Option A', 'Option B', 'Correct Answer']
                    if not all(col in df.columns for col in required_cols):
                        flash('Excel file missing required columns (Question, Option A, Option B, Correct Answer) or not a supported format', 'danger')
                        return redirect(request.url)
                ...
                for row_idx, row in df.iterrows():
                    ...
                    # Validate required fields depending on format
                    is_theory = False
                    q_text = None
                    if is_theory_only_format:
                        # Accept either 'THEORY' column or case variants
                        # find the actual column name for THEORY
                        theory_col = None
                        for c in df.columns:
                            if str(c).strip().upper() == 'THEORY':
                                theory_col = c; break
                        if theory_col is None or pd.isna(row.get(theory_col)) or str(row.get(theory_col)).strip() == '':
                            errors.append((excel_row, 'Missing theory text'))
                            continue
                        is_theory = True
                        q_text = str(row.get(theory_col)).strip()
                    else:
                        ...
                        # Detect explicit Is Theory flag per-row (optional)
                        try:
                        ...
                        if is_theory:
                            # theory content should be in 'Theory' column
                    ...
                    try:
                        ...
                        if 'Subject Code' in df.columns and not pd.isna(row.get('Subject Code')):
                            ...
                            if subj:
                                # If the found subject belongs to a different school than the uploader,
                                # create a new subject record for the uploader's school so questions
                                # remain scoped to the uploader's school.
                                try:
                                    uploader = User.query.get(session.get('user_id'))
                                    uploader_school = getattr(uploader, 'school_id', None) if uploader else None
                                    creator = User.query.get(subj.created_by) if subj.created_by else None
                                    creator_school = getattr(creator, 'school_id', None) if creator else None
                                    if uploader_school is not None and creator_school is not None and uploader_school != creator_school:
                                        # Avoid duplicating subject.code (unique). Create a local copy without the shared code.
                                        new_subj = Subject(name=(subj.name or sc), code=None, subject_class=getattr(subj, 'subject_class', None), created_by=session.get('user_id'))
                                        db.session.add(new_subj)
                                        db.session.commit()
                                        subject_for_row = new_subj.id
                                    elif uploader_school is not None and creator_school is None and uploader_school is not None:
                                        # creator has no school; create a local copy for uploader (leave code None to avoid conflicts)
                                        new_subj = Subject(name=(subj.name or sc), code=None, subject_class=getattr(subj, 'subject_class', None), created_by=session.get('user_id'))
                                        db.session.add(new_subj)
                                        db.session.commit()
                                        subject_for_row = new_subj.id
                                ...
                                # Try interpret Subject Code as an integer primary key first
                                try:
                                    sid_try = int(sc)
                                    subj = Subject.query.get(sid_try)
                                    if subj:
                                        subject_for_row = subj.id
                                    else:
                                        # create a subject record with this code to allow the upload to proceed
                                        subj = Subject(name=sc, code=sc, created_by=session.get('user_id'))
                                        db.session.add(subj)
                                        db.session.commit()
                                        subject_for_row = subj.id
                                except Exception:
                    ...
                    # Build insert record; handle theory-only format mapping
                    if is_theory_only_format:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                # Create a QuestionSet record for this upload so the import remains standalone
                try:
                    set_name = (request.form.get('set_name') or '') .strip() or filename
                except Exception:
                    set_name = filename
                question_set = None
                try:
                    question_set = QuestionSet(name=set_name, subject_id=subject_id, created_by=session.get('user_id'), upload_filename=filename)
                ...
                for rdata in rows_to_insert:
                    ...
                    sc = (rdata.get('subject_class') or subject_class) or None
                    try:
                        if not sc:
                    ...
                    # attach to this upload's question set if available
                    try:
                        if question_set and getattr(question_set, 'id', None):
                ...
                # attempt to persist traceback for debugging
                try:
                    log_dir = app.config.get('UPLOAD_FOLDER', 'uploads')
                    os.makedirs(log_dir, exist_ok=True)
                    with open(os.path.join(log_dir, 'last_upload_traceback.txt'), 'w', encoding='utf-8') as _f:
                        _f.write(tb)
                except Exception:
                    pass
                print(tb)
...
def serve_uploads(filename):
    ...
    # If `filename` is a full URL (e.g. S3 URL), redirect there instead of serving from disk.
    if filename.startswith('http://') or filename.startswith('https://'):
        return redirect(filename)
    base = os.path.abspath(app.config.get('UPLOAD_FOLDER', 'uploads'))
    # normalize filename to forward slashes and strip any leading uploads/ segment
    safe_fname = filename.replace('\\', '/').lstrip('/')
    if safe_fname.startswith('uploads/'):
        safe_fname = safe_fname[len('uploads/'):]
    target = os.path.abspath(os.path.join(base, safe_fname))
    if not target.startswith(base):
        return 'Access denied', 403
    if not os.path.exists(target):
        return 'Not found', 404
    # try to guess mimetype so images are rendered in-browser instead of downloaded
    mtype, _ = mimetypes.guess_type(target)
    if not mtype:
        mtype = 'application/octet-stream'
    return send_file(target, mimetype=mtype, conditional=True)
...
def admin_community_moderate():
    ...
    try:
        ...
        posts = []
    return render_template('admin/community_moderate.html', posts=posts)
...
def community_like(post_id):
    # Toggle like/unlike tracked per-session to avoid duplicate likes from same browser.
    try:
        post = CommunityPost.query.get(post_id)
        if not post:
            flash('Post not found', 'danger')
        ...
        ref = request.referrer or url_for('community')
        return redirect(ref)
    except Exception:
        try:
            db.session.rollback()
            ...
            pass
        flash('Failed to update like', 'danger')
        return redirect(url_for('community'))
...
@app.route('/admin/question/<int:question_id>/image', methods=['GET', 'POST'])
def admin_question_image(question_id):
    if 'user_id' not in session or session.get('role') != 'admin':
        flash('Access denied', 'danger')
        return redirect(url_for('login'))
    ...
    # Ensure admin can only modify questions belonging to their school (unless superadmin)
    try:
        if not session.get('is_superadmin'):
    ...
    if request.method == 'POST':  
        ...
        if qf and qf.filename:
            try:
                qfn = secure_filename(qf.filename)
                dest = os.path.join(app.config.get('UPLOAD_FOLDER', 'uploads'), 'question_images')
                os.makedirs(dest, exist_ok=True)
                qpath = os.path.join(dest, qfn)
                qf.save(qpath)
                question.question_image = os.path.relpath(qpath, app.config['UPLOAD_FOLDER']).replace('\\', '/')
    ...
    return render_template('admin/add_question_image.html', question=question)


@app.route('/admin/question/<int:question_id>/image/delete', methods=['POST'])
def admin_delete_question_image(question_id):
    ...
    try:
        if question.question_image:
            ...
            rel = question.question_image.replace('/', os.path.sep).lstrip(os.path.sep)
            target = os.path.abspath(os.path.join(base, rel))
            if target.startswith(base) and os.path.exists(target):
                try:
                    os.remove(target)
                except Exception:
                    pass
...
def download_question_template():
    ...
    # Define template columns (MCQ only - remove theory and image filename columns)
    cols = [
...
@app.route('/admin/question/template_theory')
def download_question_template_theory():
    # Only admin can download template
    if 'user_id' not in session or session.get('role') != 'admin':
        flash('Access denied', 'danger')
        return redirect(url_for('login'))
    ...
    return send_file(
        bio,
        as_attachment=True,
        download_name='theory_questions_template.xlsx',
        mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    )
...
def generate_questions():
    ...
    if OPENAI_KEY:
        ...
        try:
            ...
            if resp and resp.get('choices'):
                ...
                openai_error = openai_error or 'No response from OpenAI after retries.'
    ...
    if not generated:
        # surface OpenAI errors to admin so they can diagnose (don't include keys)
        if openai_error:
            flash('AI generation failed: ' + (openai_error[:200] or 'see server logs'), 'warning')
...
def upload_passport():
    ...
    try:
        user = User.query.get(session['user_id'])
        user.passport_filename = os.path.relpath(path, app.config['UPLOAD_FOLDER']).replace('\\', '/')
        db.session.commit()
...
def admin_upload_recording(session_id):
    ...
    f.save(path)
    try:
        # allow admin to optionally pass a class label when uploading
        student_class = request.form.get('student_class') if request.form else None
        rec = Recording(exam_session_id=session_id, filename=os.path.relpath(path, app.config['UPLOAD_FOLDER']).replace('\\', '/'), student_class=student_class)
        db.session.add(rec)
        db.session.commit()
...
def student_upload_recording(session_id):
    ...
    f.save(path)
    try:
        # tag recording with the student's class (if available)
        try:
            uploader = User.query.get(session.get('user_id'))
            stu_class = uploader.student_class if uploader else None
        except Exception:
            stu_class = None
        rec = Recording(exam_session_id=session_id, filename=os.path.relpath(path, app.config['UPLOAD_FOLDER']).replace('\\', '/'), student_class=stu_class)
        db.session.add(rec)
        db.session.commit()
        return {'status':'ok','recording_id':rec.id}
    except Exception as e:
...
def admin_view_exam(exam_id):
        ...
        if sids:
            ...
            for rec in raw_recs:
                ...
                recordings.append({
                    ...
                    'student_username': getattr(student, 'username', None) if student else None,
                    'student_full_name': getattr(student, 'full_name', None) if student else None,
                    'student_class': getattr(student, 'student_class', None) if student else getattr(rec, 'student_class', None),
    ...
    @app.route('/admin/exam/<int:exam_id>/recordings/delete', methods=['POST'])
    def admin_delete_recordings(exam_id):
        if 'user_id' not in session or session.get('role') != 'admin':
            flash('Access denied', 'danger')
            return redirect(url_for('login'))

        ids = request.form.getlist('recording_id')
        if not ids:
            flash('No recordings selected', 'info')
            return redirect(url_for('admin_exam_detail', exam_id=exam_id))
        ...
        for rid in ids:
            try:
                ...
                if rec:
                    ...
                    try:
                        fp = os.path.join(app.config.get('UPLOAD_FOLDER','uploads'), rec.filename)
                ...
                pass
        db.session.commit()
        flash(f'Deleted {deleted} recording(s)', 'success')
        return redirect(url_for('admin_exam_detail', exam_id=exam_id))
        ...
        if 'exam_image' in request.files:
            ...
            if f and f.filename:
                try:
                    ...
                    f.save(path)
                    exam.exam_image = os.path.relpath(path, app.config['UPLOAD_FOLDER']).replace('\\', '/')
                    db.session.add(exam)
                    db.session.commit()
...
def add_exam():
    ...
    if request.method == 'POST':
        ...
        # collect allowed classes (multi-select)
        allowed = request.form.getlist('allowed_classes') if request.form else []
        allowed_classes = ','.join([a.strip() for a in allowed if a and a.strip()]) or None

        # Optionally allow creating exam from a specific uploaded question set
        qset_id = request.form.get('question_set_id')
        try:
    ...
    # Provide available question sets to template (so admin can pick an uploaded set)
    try:
    ...
    try:
        for s in subjects:
            qlist = Question.query.filter_by(subject_id=s.id).all()
            total_count = len(qlist)
            total_marks = sum(int(q.marks or 0) for q in qlist)
            by_class = {}
            try:
                for q in qlist:
                    cls = (q.subject_class or '').strip()
                    if not cls:
                        continue
                    if cls not in by_class:
                        by_class[cls] = {'count': 0, 'marks': 0}
                    by_class[cls]['count'] += 1
                    by_class[cls]['marks'] += int(q.marks or 0)
            except Exception:
                by_class = {}
            subject_stats[s.id] = {'count': total_count, 'marks': total_marks, 'by_class': by_class}
    except Exception:
        subject_stats = {}
    ...
    try:
        for qs in question_sets:
            qlist = Question.query.filter_by(question_set_id=qs.id).all()
            total_count = len(qlist)
            total_marks = sum(int(q.marks or 0) for q in qlist)
            by_class = {}
            try:
                ...
                by_class = {}
            qset_stats[qs.id] = {'count': total_count, 'marks': total_marks, 'by_class': by_class}
    except Exception:
        qset_stats = {}
...
def student_dashboard():
    ...
    # Filter exams so only those available to the student's class are shown
    try:
        student_class = (student.student_class or '').strip()
    except Exception:
        student_class = ''

    def exam_visible_to_student(exam):
        try:
        ...
        # Fallback to subject_class if allowed_classes not set
        subj_class = ''
        try:
        ...
        if not subj_class:
            try:
                subj_class = (exam.subject.subject_class or '').strip() if exam and exam.subject else ''
            except Exception:
                subj_class = ''
...
def start_exam():
    ...
    # Ensure exam is intended for student's class (if exam has allowed_classes or subject_class restriction)
    try:
        # check allowed_classes first (comma-separated)
        allowed_raw = (getattr(exam, 'allowed_classes', None) or '')
        allowed = [a.strip().lower() for a in allowed_raw.split(',') if a.strip()]
        student_cls = (student.student_class or '').strip().lower()
        if allowed and student_cls and student_cls not in allowed:
            flash('This exam is not for your class.', 'danger')
            return redirect(url_for('start'))
        # fallback to single subject_class if no allowed_classes specified
        if not allowed:
            exam_cls = (exam.subject_class or '').strip().lower()
            if exam_cls and student_cls and exam_cls != student_cls:
                flash('This exam is not for your class.', 'danger')
                return redirect(url_for('start'))
    except Exception:
        pass
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
    # Ensure QuestionSet table and question.question_set_id column exist before serving
    try:
        # create any missing tables (QuestionSet) and then ensure column exists
        try:
            db.create_all()
        except Exception:
            pass
        # Check if `question_set_id` column exists; if not, add it (SQLite supports ADD COLUMN)
        try:
            col_info = None
            try:
                res = db.session.execute("PRAGMA table_info('question')").fetchall()
                col_info = [r[1] for r in res]
            except Exception:
                col_info = None
            if col_info is None or 'question_set_id' not in col_info:
                try:
                    db.engine.execute('ALTER TABLE question ADD COLUMN question_set_id INTEGER')
                    print('Added question.question_set_id column at startup')
                except Exception as _:
                    try:
                        db.session.rollback()
                    except Exception:
